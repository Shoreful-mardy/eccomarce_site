...
class VendorOrderController extends Controller
{
    ...
    public function VendorApprovedReturnOrder(){

        $id = Auth::user()->id;
        $orderitem = OrderItem::with('order')->where('vendor_id', $id)->orderBy('id','DESC')->get();
        return view('vendor.backend.orders.approved_return_orders',compact('orderitem'));
    ...
    public function VendorOrderDetails($order_id){
        ...
        $orderItem = OrderItem::with('product')->where('order_id', $order_id)->where('vendor_id',$vendorId)->orderBy('id','DESC')->get();
